...
## Practice 5: Avro Schema with key
Avro schemas help to regulate the format/contract of the messages and keys produced to kafka topics

1. Create avro key schema (used for key)
```
{
  "name": "Key",
  "type": "int"
...
```
docker-compose run --rm services avro-schema-with-key--produce-messages <topic_name>
```
- Please replace <topic_name> with the desired topic name. ex: topic1
- ex: _docker-compose run --rm services avro-schema-with-key--produce-messages topic1_
- 2 different messages will be produced. Each message will be produced 5 times
- Id generated by `random.randrange` will be used as key
- AvroProducer will be used. Both value and key schemas should be loaded in advance: `avro.load(<path_to_schema>)`
...
4. Check the key schema in the schema registry
    ...
    curl --silent -X GET http://localhost:18081/subjects/<schema_name>/verions
    ```
    - ex: _curl --silent -X GET http://localhost:18081/subjects/topic1-key/versions_
    
    iii. Get a certain version of a certain schema
    ```
    curl --silent -X GET http://localhost:18081/subjects/<schema_name>/verions/<version_id>
    ```
    - ex: _curl --silent -X GET http://localhost:18081/subjects/topic1-key/versions/1_
