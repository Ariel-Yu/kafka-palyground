...
def test_create_avro_producer(mocker):
    mocker.patch("project_kafka.infrastructure.factories.producer_factories.AvroProducer.__init__", return_value=None)
    assert isinstance(create_avro_producer("schema"), AvroProducer)


def test_create_producer(mocker):
    mocker.patch("project_kafka.infrastructure.factories.producer_factories.Producer.__init__", return_value=None)
